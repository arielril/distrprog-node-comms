...
class Node:
    ...
    def register_to_supernode(self, app):
        try:
            ...
            app.logger.debug(f"[+] node ({self.node_name}) is registering.")
            ...
            if res.status_code == 204:
                app.logger.info(f"[+] node ({self.node_name}) is registered")
            else:
                app.logger.warn(f"[!] node ({self.node_name}) is not registered, something happend", res.status_code)
        except Exception as e:
            app.logger.error(f"[!] node ({self.node_name}) is not registered, request failed.", e)

    def load_resources(self, path=""):
        if not isdir(path):
            raise NotADirectoryError(f'"{path}" is not a directory')

        resource_files = [f for f in listdir(path) if isfile(join(path, f))]
    ...
    def supernode_search_resource(cls, id: str):
        try:
            ...
            if res.status_code != 200:
                current_app.logger.error(f"[!] error searching for resource ({id}) at supernode")
                return None

            res_data = res.json()

            resource_info = None
            if "data" in res_data and "file" in res_data["data"]:
                resource_info = res_data["data"]["file"]

            return resource_info
        except Exception as e:
            current_app.logger.error(f"[.] failed to search resource ({id}) at supernode.", e)
